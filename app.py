...
def serve_layout():
    ...
    if current_user.is_authenticated:
        ...
        html.Form(
            ...
            action="/signup", 
            children=[
                html.Label(children=["ユーザー名: ", dcc.Input(type='text', name='username', required=True),]),
                html.Br(),
                html.Label(children=["メールアドレス: ", dcc.Input(type='email', name='email'),]),
                html.Br(),
                html.Label(children=["パスワード: ", dcc.Input(type='password', name='password', required=True),]),
                html.Br(),
                html.Label(children=["役割 (Role): ", html.Select(
                    children=[
                        html.Option("Doctor", value="doctor"),
                        html.Option("Admin", value="admin"),
                    ],
                    name="role",
                    required=True,
                ),]),
                html.Br(),
                html.Button("サインアップ", type="submit")
            ]
        ),
...
def do_signup():
    ...
    try:
        user_id = create_user(username, password, role, email=email if email else None)
        flash("ユーザー登録が成功しました！ログインしてください")
        return redirect("/login")
    except ValueError as e:
        flash(str(e))  # Example: "username exists"
        return redirect("/")
    except Exception as e:
        server.logger.error(f"Error creating user: {e}")
        flash("サーバーエラーが発生しました。後でもう一度お試しください。")
        return redirect("/")
'''
